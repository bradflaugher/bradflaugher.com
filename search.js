document.addEventListener('DOMContentLoaded', () => {
  ...
  // Web search function
  window.doSearch = function(engine) {
    if (!search) return;
    ...
    // For Maps on iOS, attempt to open the app
    ...
    // Standard behavior: Open in the system's default/current browser
  ...
  document.querySelectorAll('.web-btn').forEach(btn => {
    btn.addEventListener('click', () => {
      window.doSearch(btn.dataset.engine);
    });
  });
